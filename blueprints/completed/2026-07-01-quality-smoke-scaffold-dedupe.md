...
The root Playwright quality scaffold lived in consumer-owned files: `playwright.config.ts`, `e2e/smoke.spec.ts`, and `e2e/fixtures/smoke.html`. Those files duplicated the generic quality smoke contract already extracted to `@webpresso/agent-config@0.3.3`.

## Architecture after

The root Playwright quality scaffold is selected through the public `@webpresso/agent-config/playwright/quality-scaffold` helper. Consumer-local E2E behavior remains under `apps/e2e`, while the deleted root scaffold paths are ignored as setup-owned materializations rather than repo-maintained source.
