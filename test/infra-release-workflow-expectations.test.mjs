...
test('workflow governance directory exists', () => {
  ...
  assert.ok(workflows.length > 0, 'expected at least one GitHub Actions workflow under .github/workflows')
  assert.ok(
    workflows.includes(PR_CI_WORKFLOW),
    `expected PR CI workflow at ${PR_CI_WORKFLOW}`,
  )
})
...
test('PR CI workflow includes quality gates, build, docs checks, and dry-run deploy', () => {
  ...
  assert.equal(
    missing.length,
    0,
    formatMissingExpectations(missing, PR_CI_WORKFLOW),
  )
})
...
test('production deploy workflow serializes deploys and runs smoke verification', () => {
  ...
  assert.equal(
    missing.length,
    0,
    formatMissingExpectations(missing, PRODUCTION_DEPLOY_WORKFLOW),
  )
})
