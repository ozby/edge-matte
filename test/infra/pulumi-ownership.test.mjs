...
test('Pulumi storage program provisions the bucket referenced by Wrangler', () => {
  ...
  assert.ok(
    storagePath,
    `expected a Pulumi storage module (${storageCandidates.join(' or ')})`,
  )
...
test('Pulumi owns lifecycle cleanup for stale job artifacts', () => {
  ...
  const lifecyclePath = lifecycleCandidates.find((candidate) => existsSync(resolve(root, candidate)))
  assert.ok(
    lifecyclePath,
    'expected Pulumi lifecycle rules for failed/intermediate R2 objects',
  )
